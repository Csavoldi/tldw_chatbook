...
class LibraryMediaViewer(PostRecomposeCallback, Vertical):
    ...
    def _compose_primary_toolbar(self) -> ComposeResult:
        ...
        if self.more_open and (not self.external_detail or self.viewer.original_source):
            ...
            # onto a second row instead. Reserve the longest label (13),
            # two cells of gutter, and the destructive action's two-cell
            # left margin so its full "Move to trash" label remains visible.
            with ItemGrid(
